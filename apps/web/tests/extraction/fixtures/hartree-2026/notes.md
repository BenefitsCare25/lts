...
**Source:** Anonymized from Hartree Partners Singapore + CHC Energy's real placement slip dated 2026-2027.
...
8 products / 3 insurers / 2 entities. Smaller than VDL but introduces several **firsts** in the corpus:
...
- **Three insurers across three different policy structures:**
  - HSBC Life Insurance (6 products, Insurope Pool, single policy `BGDC211000450-04-000` shared across GTL/GCI/GHS/GP/SP/Dental)
...
- **Three different business descriptions across sheets:**
  - GTL/GCI: "Wholesale Of Fuels And Related Products" (Hartree's commodity trading)
  - GHS/GP/SP/Dental/GBT: "ACTIVITIES OF HEAD AND REGIONAL HEAD OFFICES; CENTRALISED ADMINISTRATIVE OFFICES AND SUBSIDIARY MANAGEMENT OFFICES" (CHC Energy's HQ activities)
  - WICA: "Battery Energy Storage Systems" (CHC Energy's operating activity)
...
2. **UEN `202239963W` → `T26TST0001A`.** The slip's WICA sheet leaks CHC Energy's UEN in B20/B28 cells. Standard SG UEN format preserved.

3. **Japan affiliate name redacted but country reference kept.** `CHC Japan K.K. (Kabushiki-Kaisha)` → `Test E Japan K.K.` The "Japanese" employee references in GBT B177 are kept (generic — many SG firms employ Japanese nationals); the affiliate's identity is the part that's anonymized.
...
5. **Catch-all bare `Hartree` and `CHC` replacements.** Run LAST (after longer phrase replacements) to catch any leftover bare references.
