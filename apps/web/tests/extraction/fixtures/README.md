...
> - **Numeric policy IDs need `cell_overrides`, not `replace_strings`.** The string-replace pass skips non-string cells. If a policy number is stored as an integer (e.g. Tokio Marine's 50011843), use a per-cell override.
...
> - **Pseudonyms can self-leak.** `TEST/AW/PNG-002` decodes back to "Papua New Guinea". Use neutral patterns like `TEST/<insurer>/2026-NNN`.
