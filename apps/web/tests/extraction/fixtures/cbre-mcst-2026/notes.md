...
**Source:** Anonymized from CBRE MCST 1216's real placement slip dated 2025-2026.
...
- **`Policyholder(s) Rated Together: CBRE Group`** — the original slip leaks the parent group name in this row. The anonymization replaces with `Test Group`. Per-fixture audit log records the rule.
...
- Insurer code mapping: the AI returned `TM_LIFE`-style codes for STMicro's GE-Life products. For Tokio Marine Life the canonical code should also follow the registry. Verify after Phase 2 V-7 fix lands.
