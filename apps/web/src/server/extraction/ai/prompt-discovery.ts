...
export function buildDiscoveryUserPrompt(
  ...
  return [
    ...
    '- proposedTpa: look for a row labelled "TPA", "Third Party Administrator", or',
    '  "Claims Administrator" on any product sheet. Common Singapore TPA names include',
    '  "Adept Health", "AXA IHO", "Integrated Health Plans (IHP)", "Alliance Healthcare",',
    '  "Fullerton Health". Treat "NA", "N.A", "N/A", "NIL" as no TPA. Set null when absent.',
    '  When found, set `rawLabel` to the verbatim cell value. Leave `tpaId` null — the broker',
    '  will bind it to the registry.',
